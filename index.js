...
client.on('ready', () => {
  const channel = client.channels.cache.get("1016570076708737054");
  if (channel) {
    channel.join()
    setInterval(() => {
      channel.join
...
client.on("message", async(message) => {
  if (message.content.startsWith(prefix + "setavatar")) {
      ...
      .slice(1)
      .join(' ');
    if (message.author.id !== "742782298797113425") return;
    try {
      client.user.setAvatar(args[0]).catch((e) => console.log(e))
      message.react("✅")
    } catch (e) {
      null
    }
